...
│   ├── difficulty_phases.py  # 难度阶段系统
│   └── weight_calculator.py  # 动态权重计算引擎
├── scenes/              # 🎬 游戏场景
│   ├── __init__.py
│   ├── registry.py      # 场景工厂注册表
│   ├── base_scene.py    # 场景基类
...
├── debug/               # 🔍 调试探针系统（F1开关）
│   ├── __init__.py
│   ├── probe_core.py    # 数据收集核心
│   ├── probe_panel.py   # UI面板（滚动+缩放）
│   └── probe_window.py  # 独立pygame窗口
├── utils/               # 🛠️ 工具模块
│   ├── __init__.py
