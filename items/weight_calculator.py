...
def calc_length_modifier(category: str, snake_length: int) -> float:
    ...
    障碍物/减益: 1.0 + (蛇长-3) × 0.05
      蛇长  3 → ×1.0   蛇长 13 → ×1.5   蛇长 23 → ×2.0
    幸运类: max(0.5, 1.0 - (蛇长-3) × 0.02)
      蛇长  3 → ×1.0   蛇长 13 → ×0.8   蛇长 28 → ×0.5
    """
    length_factor = max(0, snake_length - 3)
    if category in (CAT_OBSTACLE, CAT_DEBUFF):
        return 1.0 + length_factor * 0.05
    elif category == CAT_LUCKY:
        return max(0.5, 1.0 - length_factor * 0.02)
    return 1.0
...
def calc_score_modifier(category: str, score: int, early_max_score: int) -> float:
    ...
    score < early_max:  ×0.1（几乎不出）
    score ≥ early_max:  min(2.0, 0.1 + (score - early_max) / early_max)
    """
    if category in (CAT_LUCKY, CAT_BUFF):
        if score < early_max_score:
            return 0.1
        return min(2.0, 0.1 + (score - early_max_score) / early_max_score)
    return 1.0
